...
class DUTSyncFIFO(object):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.wr_en = xsp.XPin(xsp.XData(0, xsp.XData.In), self.event)
        self.rd_en = xsp.XPin(xsp.XData(0, xsp.XData.In), self.event)
        self.wdata = xsp.XPin(xsp.XData(8, xsp.XData.In), self.event)
        self.rdata = xsp.XPin(xsp.XData(8, xsp.XData.Out), self.event)
        self.empty = xsp.XPin(xsp.XData(0, xsp.XData.Out), self.event)
        self.full = xsp.XPin(xsp.XData(0, xsp.XData.Out), self.event)
        ...
        self.wr_en.BindDPIPtr(self.dut.GetDPIHandle("wr_en", 0), self.dut.GetDPIHandle("wr_en", 1))
        self.rd_en.BindDPIPtr(self.dut.GetDPIHandle("rd_en", 0), self.dut.GetDPIHandle("rd_en", 1))
        self.wdata.BindDPIPtr(self.dut.GetDPIHandle("wdata", 0), self.dut.GetDPIHandle("wdata", 1))
        self.rdata.BindDPIPtr(self.dut.GetDPIHandle("rdata", 0), self.dut.GetDPIHandle("rdata", 1))
        self.empty.BindDPIPtr(self.dut.GetDPIHandle("empty", 0), self.dut.GetDPIHandle("empty", 1))
        self.full.BindDPIPtr(self.dut.GetDPIHandle("full", 0), self.dut.GetDPIHandle("full", 1))


        # Add2Port
        self.xport.Add("clk", self.clk.xdata)
        self.xport.Add("rst_n", self.rst_n.xdata)
        self.xport.Add("wr_en", self.wr_en.xdata)
        self.xport.Add("rd_en", self.rd_en.xdata)
        self.xport.Add("wdata", self.wdata.xdata)
        self.xport.Add("rdata", self.rdata.xdata)
        self.xport.Add("empty", self.empty.xdata)
        self.xport.Add("full", self.full.xdata)
